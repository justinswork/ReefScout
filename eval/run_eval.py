...
# Hand-maintained narrative appended after the auto-generated table on every full run,
# so the report is always complete (and the iteration story survives a re-run).
HISTORY_MD = """
## Run history
...
CASES: list[dict] = [
    ...
    {
        "id": "id_shows_photo",
        "why": "Identification with a photo request must fetch a reference image and embed it with attribution.",
        "message": "I saw a bright orange fish about a foot long at La Jolla. Show me a photo to confirm.",
        "checks": [
            ("tools_include", ["get_species_images"]),
            ("reply_matches", r"!\[[^\]]*\]\(https://[^\s)]+\)"),  # embedded markdown image
            ("reply_matches", r"©|\(c\)|attribution|uploaded by"),  # credit present
        ],
    },
    {
        "id": "off_topic",
